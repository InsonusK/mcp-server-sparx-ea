...
its ADR only names tools for C#/.NET, Angular/TypeScript and Python. This project is Go, so the Go choice has to be recorded here.
...
- One `gremlins unleash` run per production package (`./client/eaxmi`,
  `./internal/service/sparx`, `./internal/mcpserver`) — gremlins accepts a
  single path per invocation.
...
AST-based, older, less active. No first-class JSON report. Rejected.
...
- `make mutation-test` is slow (~40 s here) because every mutant reruns the full
  ...
  (`--diff`).
- The whole codebase is pure Go and mutated by the Cucumber suites; there is
  no cgo or generated code to exclude.
