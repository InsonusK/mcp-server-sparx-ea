Feature: Element information (method 2)
  As the MCP server
  I want an element's ArchiMate type, note and relationships by id or by path
  So that an agent can inspect a single element without walking the whole model

  Background:
    Given the model file "TestProject.xml"

  Scenario: Reading an element by path returns its type, note and identity
  ...
  Scenario: Reading a Cyrillic element
  ...
  Scenario: Reading a missing element is an error
    ...
    Then reading fails with "no element for"
